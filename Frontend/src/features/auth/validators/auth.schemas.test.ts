...
describe('authentication validation contracts', () => {
  it('requires backend-compatible email, name, and password lengths', () => {
    ...
    expect(profileSchema.safeParse({ firstName: 'A'.repeat(101), lastName: 'Khan' }).success).toBe(
      false,
    )
  })
  ...
  it('keeps password confirmations client-side and validates equality', () => {
    expect(
      resetPasswordSchema.safeParse({ newPassword: 'new-password', confirmPassword: 'different' })
        .success,
    ).toBe(false)
    expect(
