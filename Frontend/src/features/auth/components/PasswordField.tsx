    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="relative">
        <Input
          ...
          className={error ? 'border-accent-coral pr-14' : 'pr-14'}
          {...props}
        />
        <Button
          type="button"
      ...
      {error ? (
        <p id={messageId} className="flex items-center gap-2 text-sm text-accent-coral" role="alert">
          <AlertCircle aria-hidden="true" size={16} /> {error}
        </p>
      ) : hint ? <p id={messageId} className="text-sm text-muted-foreground">{hint}</p> : null}
    </div>
  )
})
