...
public class ShareLinkController : ControllerBase
{
    ...
    public async Task<ActionResult<GenerateShareLinkResponseDto>> GenerateShareLink(
        [FromBody] GenerateShareLinkRequestDto request,
        CancellationToken cancellationToken)
    {
        var userId = GetAuthenticatedUserId();

        var shareLink = await _shareLinkService.GenerateAsync(
            request,
            userId,
            cancellationToken);

        var response = ToGenerateShareLinkResponse(
            shareLink,
            forceActiveLink: true);
    ...
    public async Task<ActionResult<IEnumerable<GenerateShareLinkResponseDto>>> GetOwnShareLinks(
        CancellationToken cancellationToken)
    {
        var userId = GetAuthenticatedUserId();

        var shareLinks = await _shareLinkService.GetOwnerLinksAsync(
            userId,
            cancellationToken);

        var response = shareLinks
            .Select(shareLink => ToGenerateShareLinkResponse(shareLink))
            .ToList();
    ...
    public async Task<ActionResult<MessageResponseDto>> RevokeShareLink(
        Guid shareLinkId,
        CancellationToken cancellationToken)
    {
        var userId = GetAuthenticatedUserId();

        var response = await _shareLinkService.RevokeAsync(
            shareLinkId,
            userId,
            cancellationToken);
    ...
    public async Task<IActionResult> PreviewSharedFile(
        string shareToken,
        CancellationToken cancellationToken)
    {
        var result = await _shareLinkService.PreviewByTokenAsync(
            shareToken,
            cancellationToken);
    ...
    public async Task<IActionResult> DownloadSharedFile(
        ...
        CancellationToken cancellationToken)
    {
        var result = await _shareLinkService.DownloadByTokenAsync(
            shareToken,
            cancellationToken);
    ...
    private GenerateShareLinkResponseDto ToGenerateShareLinkResponse(
        ShareLinkDto shareLink,
        bool forceActiveLink = false)
    {
