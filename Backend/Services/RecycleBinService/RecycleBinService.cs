...
public class RecycleBinService : IRecycleBinService
{
    ...
    public RecycleBinService(IRecycleBinRepository recycleBinRepository, IFolderRepository folderRepository, IUserFileRepository userFileRepository, IPhysicalStorageProvider physicalStorageProvider, IStorageAccountService storageAccountService, IStorageQuotaService storageQuotaService, IUnitOfWork unitOfWork, IConfiguration configuration, ILogger<RecycleBinService> logger)
    {
    ...
    public async Task<IEnumerable<RecycleBinItemDto>> GetItemsAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        return visibleFolders.Select(folder => new RecycleBinItemDto { ItemId = folder.Folderid, ItemType = "Folder", Name = folder.Name, OriginalParentFolderId = folder.Originalparentfolderid, DeletedAt = folder.Deletedat!.Value, ExpiresAt = folder.Deletedat.Value.Add(_retentionPeriod) }).Concat(visibleFiles.Select(file => new RecycleBinItemDto { ItemId = file.Fileid, ItemType = "File", Name = file.Filename, OriginalParentFolderId = file.Folderid, Extension = file.Extension, MimeType = file.Mimetype, FileSizeBytes = file.Filesizebytes, DeletedAt = file.Deletedat!.Value, ExpiresAt = file.Deletedat.Value.Add(_retentionPeriod) })).OrderByDescending(item => item.DeletedAt).ThenBy(item => item.Name).ToList();
    }

    public async Task<MessageResponseDto> RestoreFileAsync(Guid fileId, Guid userId, CancellationToken cancellationToken = default)
    {
        var file = await GetOwnedDeletedFileAsync(fileId, userId, cancellationToken);

        if (file.Folderid.HasValue)
        {
            var parentFolder = await _folderRepository.GetByIdAsync(file.Folderid.Value, cancellationToken);
    ...
    public async Task<MessageResponseDto> RestoreFolderAsync(Guid folderId, Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        folder.Parentfolderid = await GetRestoredParentFolderIdAsync(folder.Originalparentfolderid, userId, cancellationToken);
        ...
        foreach (var file in files.Where(f => f.Folderid.HasValue && folderIds.Contains(f.Folderid.Value)))
        {
            file.Isdeleted = false;
            file.Deletedat = null;
    ...
    public async Task<MessageResponseDto> PermanentlyDeleteFileAsync(Guid fileId, Guid userId, CancellationToken cancellationToken = default)
    {
        var file = await GetOwnedDeletedFileAsync(fileId, userId, cancellationToken);
    ...
    public async Task<MessageResponseDto> PermanentlyDeleteFolderAsync(Guid folderId, Guid userId, CancellationToken cancellationToken = default)
    {
        var folder = await GetOwnedDeletedFolderAsync(folderId, userId, cancellationToken);
        var allFolders = await _folderRepository.GetByUserIdAsync(userId, cancellationToken);
        ...
        foreach (var file in deletedFiles.Where(f => f.Folderid.HasValue && folderIds.Contains(f.Folderid.Value)))
        {
            await PermanentlyDeleteFileAsync(file, cancellationToken);
        }

        foreach (var folderToDelete in foldersToDelete.OrderByDescending(item => GetFolderDepth(item, allFolders)))
        {
            _folderRepository.Remove(folderToDelete);
        }

        await _unitOfWork.SaveChangesAsync(cancellationToken);
        _logger.LogInformation("Permanently deleted folder {FolderId} and its contents for owner {OwnerId} from the Recycle Bin.", folder.Folderid, folder.Ownerid);
    ...
    public async Task<int> DeleteExpiredItemsAsync(CancellationToken cancellationToken = default)
    {
        ...
        var expiredFolders = (await _recycleBinRepository.GetExpiredDeletedFoldersAsync(expiryCutoff, cancellationToken)).ToList();
        var expiredFolderIds = expiredFolders.Select(folder => folder.Folderid).ToHashSet();
        var expiredFiles = await _recycleBinRepository.GetExpiredDeletedFilesAsync(expiryCutoff, cancellationToken);
        var deletedCount = 0;

        foreach (var folder in expiredFolders.Where(folder => !folder.Parentfolderid.HasValue || !expiredFolderIds.Contains(folder.Parentfolderid.Value)))
        {
            try
            {
    ...
    {
        await _physicalStorageProvider.DeleteAsync(file.Storageaccountid, file.Providerobjectid, cancellationToken);

        await _storageAccountService.ReleaseCapacityAsync(file.Storageaccountid, file.Filesizebytes, cancellationToken);

        await _storageQuotaService.ReleaseStorageAsync(file.Ownerid, file.Filesizebytes, cancellationToken);

        _userFileRepository.Remove(file);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
        _logger.LogInformation("Permanently deleted file {FileId} for owner {OwnerId} from the Recycle Bin.", file.Fileid, file.Ownerid);
    }

    private async Task<Userfile> GetOwnedDeletedFileAsync(Guid fileId, Guid userId, CancellationToken cancellationToken)
    {
        ...
        if (file.Folderid.HasValue)
        {
            ...
            if (parentFolder is not null && parentFolder.Ownerid == userId && parentFolder.Isdeleted)
            {
                throw new KeyNotFoundException("Recycle Bin file not found.");
            }
        }
    ...
    private async Task<Folder> GetOwnedDeletedFolderAsync(Guid folderId, Guid userId, CancellationToken cancellationToken)
    {
        ...
        {
            var parentFolder = await _folderRepository.GetByIdAsync(folder.Parentfolderid.Value, cancellationToken);
    ...
    private async Task<Guid?> GetRestoredParentFolderIdAsync(Guid? originalParentFolderId, Guid userId, CancellationToken cancellationToken)
    {
        ...
        var parentFolder = await _folderRepository.GetByIdAsync(originalParentFolderId.Value, cancellationToken);

        return parentFolder is not null && parentFolder.Ownerid == userId && !parentFolder.Isdeleted
            ? parentFolder.Folderid
            : null;
    }

    private static List<Folder> GetFolderTree(Folder rootFolder, IEnumerable<Folder> folders)
    {
        var foldersByParentId = folders.Where(folder => folder.Parentfolderid.HasValue).GroupBy(folder => folder.Parentfolderid!.Value).ToDictionary(group => group.Key, group => group.ToList());
    ...
    private static int GetFolderDepth(Folder folder, IEnumerable<Folder> folders)
    {
        ...
        while (currentFolder.Parentfolderid.HasValue && foldersById.TryGetValue(currentFolder.Parentfolderid.Value, out var parentFolder))
        {
            depth++;
